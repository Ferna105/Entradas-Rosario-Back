#!/usr/bin/env bash
# Ejecuta docker/migration_attendees.sql contra la base indicada.
#
# Uso (elige una):
#   1) URL de conexión:
#        export DATABASE_URL="postgresql://user:pass@host:5432/dbname"
#        ./scripts/run-attendees-migration.sh
#
...
#        ./scripts/run-attendees-migration.sh /ruta/al/.env.production
#
# Es idempotente: agrega columnas sólo si no existen.

set -euo pipefail

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
BACKEND_ROOT="$(cd "$SCRIPT_DIR/.." && pwd)"
MIGRATION="$BACKEND_ROOT/docker/migration_attendees.sql"
